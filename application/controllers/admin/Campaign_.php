...
class Campaign extends MY_Controller
{
	...
	function save($id="")
	{
		if($id == "") // create new
		{
			...
			if($this->input->post('submit')){
				$this->form_validation->set_rules('campaign_name', 'campaign_name', 'trim|required');
				$this->form_validation->set_rules('assessment_date', 'assessment_date', 'trim|required');
				$this->form_validation->set_rules('assessment_criteria', 'assessment_criteria', 'trim|required');
				$this->form_validation->set_rules('assessment_type', 'assessment_type', 'trim|required');
				$this->form_validation->set_rules('assessment_address', 'assessment_address', 'trim|required');
				...
				if ($this->form_validation->run() == FALSE) {
					...
					$result = $this->campaign_model->add_campaign($data);
					if($result){
						// Activity Log 
	...
	function list_campaigns_json()
	{
		...
		$records['data'] = $this->campaign_model->get_all_campaign($start,$length);
		$data = array();
		$i=0;
	...
	*/
	function view($id=""){
		if($id==""){
			...
			if(!$this->campaign_model->has_the_campaign($data['record']['company_id']))
			{
				redirect('admin/campaign');
			}
			$flag = 0;
